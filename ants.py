...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            curAnt=place.ant
            if curAnt.is_container==True and self.is_container==False and curAnt.can_contain(self):
                place.ant=curAnt
                curAnt.store_ant(self)
            elif curAnt.is_container==False and self.is_container==True and self.can_contain(curAnt):
                place.ant=self
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        curPlace=self.place
        curBees=list(curPlace.bees)
        super().reduce_health(amount)
        ndamage=amount
        if self.health <= 0:
            ndamage+=self.damage
        for bee in curBees:
            bee.reduce_health(ndamage)
        # END Problem 5
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if getattr(gamestate, 'has_queen', False):
            return None
    ...
    def action(self, gamestate):
        ...
        while curPlace!=None:
            ...
            if curAnt.is_container==True:
                if not getattr(curAnt, 'is_doubled', False):
                    curAnt.is_doubled = True
                    curAnt.damage*=2
                curAnt=curAnt.ant_contained
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        curPlace = self.place
        distance = 0
        while not curPlace.is_hive:
            if curPlace.ant is not None and curPlace.ant is not self:
                insects[curPlace.ant] = distance
                if curPlace.ant.is_container and curPlace.ant.ant_contained:
                    if curPlace.ant.ant_contained is not self:
                        insects[curPlace.ant.ant_contained] = distance
            for bee in curPlace.bees:
